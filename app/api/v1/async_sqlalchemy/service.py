...
# データベースの初期化
async def init_db():
  # テーブル作成前に毎回初期状態に戻すことで、練習時に状態が不揃いになるのを防ぐ
...
async def add_user(name):
  ...
  async with async_session() as session:
    async with session.begin():
      # モデルのインスタンスを作ってセッションに追加すると、コミット時にINSERTされる
      user = User(name=name)
      session.add(user)
...
async def get_users():
  ...
  async with async_session() as session:
    # select(User) を実行すると User テーブルの全行が取得できる
    result = await session.execute(select(User))
    # scalars() で User オブジェクトだけを取り出し、all() でリスト化
...
# メイン関数
async def main():
  # このファイルを直接実行したときに流れが追いやすいよう、処理を順番に呼び出す
  await init_db()
  await add_user("田中")
  await add_user("岡田")
