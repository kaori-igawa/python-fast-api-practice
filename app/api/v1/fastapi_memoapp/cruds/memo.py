...
async def get_memos(db_session: AsyncSession) -> list[memo_model.Memo]:
  ...
  """
  print("=== 全件取得：開始 ===")
  # データベースから全てのメモを選択するSQLクエリを実行する
  result = await db_session.execute(select(memo_model.Memo))
  # scalars() → クエリ結果の各行を個別のオブジェクトとして抽出する。 
  # all() → 全てのメモオブジェクトをリストとして返す
  memos = result.scalars().all()
  print(">>> データ全件取得完了")
  return memos

# 1件取得
async def get_memo_by_id(db_session: AsyncSession, memo_id: int) -> memo_model.Memo | None:
  """
...
async def delete_memo(
  ...
  if memo:
    ...
    # データベースに変更を保存する。これによりメモの削除が確定する。
    await db_session.commit()
    print(">>> データ削除完了")
